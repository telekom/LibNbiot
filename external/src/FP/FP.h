...
public:
    ...
    /**
     * @brief FP copy constructor
     * @param other FP instance to copy
     */
    FP(const FP& other):obj_callback(0),c_callback(0)
    {
        if(other.attached())
        {
            if(other.obj_callback)
            {
                obj_callback = other.obj_callback;
                method_callback = other.method_callback;
            }
            else
            {
