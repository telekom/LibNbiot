...
const char* MQTTPacket_toString(char* buf, unsigned int buflen, const unsigned char* packet, int length)
{
    snprintf(buf, buflen, "%s (%d)", MQTTSNPacket_name((int)((255 > length)?packet[1]:packet[3])), length);
    return buf;
}
